 ...
 export class ClientsTableDataSource extends DataSource<Client> {
    ...
    connect(): Observable<Client[]> {
      ...
      // In the nutshell update displaying data when one of the events (pageEvent, sort, ordersSubject.next) has occured
        return merge(this.clientsSubject.asObservable(), this.paginator.page, this.sort.sortChange)
          .pipe(map((value) => {
    ...
    private getSortedData(data: Client[]): Client[] {
      ...
      return data.sort((a, b) => {
        ...
        switch (this.sort?.active) {
          case 'email': return this.defaultCompare(a.email, b.email, isAsc);
          case 'phone': return this.defaultCompare(a.phone, b.phone, isAsc);
          case 'address': return this.defaultCompare(a.street, b.street, isAsc);
          case 'client': return this.compareClients(a, b, isAsc);
          default: return 0;
        }
      });
    }
