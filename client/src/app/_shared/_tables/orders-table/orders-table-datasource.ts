...
export class OrdersTableDataSource extends DataSource<Order> {
  ...
  connect(): Observable<Order[]> {
    ...
    return merge(this.ordersSubject.asObservable(), this.paginator.page, this.sort.sortChange).pipe(
      map((value) => {
  ...
  private getSortedData(data: Order[]): Order[] {
    ...
    return data.sort((a, b) => {
      ...
      switch (this.sort?.active) {
        ...
        case 'orderNumber':
          return this.defaultCompare(a.orderNumber, b.orderNumber, isAsc);
        case 'client':
          return this.compareClients(a.client, b.client, isAsc);
        case 'vehicle':
        ...
        case 'createDate':
          return this.compareDates(new Date(a.createdAt), new Date(b.createdAt), isAsc);
        case 'finishDate':
          return this.compareDates(new Date(a.finishDate), new Date(b.finishDate), isAsc);
        case 'status':
          return this.defaultCompare(a.status.position, b.status.position, !isAsc);
        case 'totalGross':
          return this.defaultCompare(+a.totalGross.toFixed(2), +b.totalGross.toFixed(2), !isAsc);
        case 'admissionDate':
          return this.compareDates(new Date(a.admissionDate), new Date(b.admissionDate), isAsc);
        case 'deadlineDate':
          return this.compareDates(new Date(a.deadlineDate), new Date(b.deadlineDate), isAsc);
        default:
          return 0;
      }
    });
  ...
  addOrder(order: Order): void {
    this.ordersSubject.next([order, ...this.data]);
  }

  updateOrder(updatedOrder: Order) {
    const updatedData = this.data.map((order) =>
      order.id == updatedOrder.id ? updatedOrder : order
    );
    this.ordersSubject.next(updatedData);
  }

  deleteOrder(id: number): void {
    this.ordersSubject.next(this.data.filter((order) => order.id !== id));
  }
